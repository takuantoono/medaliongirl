...
(function () {
  ...
  Sprite_VirtualButton.prototype.update = function () {
    ...
    if (this.active&&!$gameSwitches.value(82)) {
  ...
  if(SceneManager._scene.constructor==Scene_LearnSkill){
  x+=200
  image = "ButtonSet2"
  hotImage = "ButtonSet2"
  }
  ...
  Sprite_DirectionalPad.prototype.whichIndex = function (point) {
    var index = 0;
    index = point.x - this.x > this.width / 2 ? 7: 1;
    if(SceneManager._scene.constructor==Scene_Battle){
    index = 0;
    if(point.x>this.x+30&&point.x<this.x+60&&point.y>this.y+30&&point.y<this.y+60){
    }else{
    ...
    if(SceneManager._scene.constructor==Scene_Skill||SceneManager._scene.constructor==Scene_Equip||SceneManager._scene.constructor==Scene_LearnSkill){
    if(point.x > (this.x + this.width / 3 * 2)){
    index = 2;
    }else if(point.x < this.x + this.width / 3){
    index = 1;
    }else{
    index = 7;
    }
    }
    if(SceneManager._scene.constructor==Scene_Status)index=2;
    //index += point.x - this.x > this.width / 3 ? point.x - this.x > this.width * 2 / 3 ? 2 : 1 : 0;
    //index += point.y - this.y > this.height / 3 ? point.y - this.y > this.height * 2 / 3 ? 6 : 3 : 0;
    return index;
  };
...
//aaa
  ...
  Scene_Base.prototype.start = function () {
    ...
    if(SceneManager._scene.constructor==Scene_Map)SceneManager._scene._directionalPad.hide();
  };
