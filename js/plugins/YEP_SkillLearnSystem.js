...
Game_Actor.prototype.customLearnSkillJpCost = function(skill) {
    ...
    var mcost = -1 * item.learnCostJp*4/5;
    var a = this;
    var b = this;
    var user = this;
    var target = this;
...
Window_SkillLearn.prototype.getEvalLine = function(evalLine) {
    ...
    if(mmclass){
    if(user.isClass($dataClasses[4])){
    if($gameSwitches.value(502)||$gameVariables.value(483)==14)value=true
    }
    }
    if(!$gameSwitches.value(117)&&lowR)value=false
    if(!$gameSwitches.value(118)&&middleR)value=false
    if(!$gameSwitches.value(119)&&highR)value=false
    return value;
};
...
Window_SkillLearnData.prototype.drawCustomText = function(wy) {
    ...
    var text = this.japList(1001) + ": "+ this._skill.learnRequireLevel
    this.drawTextEx(text, 4, wy);
    return wy;
};
