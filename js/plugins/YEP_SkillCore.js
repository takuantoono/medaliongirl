...
Game_BattlerBase.prototype.noHiddenSkillConditionsMet = function(skill) {
    ...
    if (skill.occasion === 2 && $gameSwitches.value(209)) return false;
    if (skill.hideInBattle && $gameParty.inBattle()) return false;
    if (skill.hideInField && !$gameParty.inBattle() && !$gameSwitches.value(209)) return false;
...
Game_BattlerBase.prototype.meetsSkillConditionsEval = function(skill) {
    ...
    if(user){
    if(user._classId==4&&!user.isStateAffected(326)&&mcl7)value=false;
    }
    var noblack = false;
    eval(skill.requireEval);
    if(user){
    if(user.isStateAffected(351)&&noblack)value=false;
    }
    var nowhite = false;
    eval(skill.requireEval);
    if(user){
    if(user.isStateAffected(352)&&nowhite)value=false;
    }
    return value;
};
...
Game_Action.prototype.apply = function(target) {
	if(this.subject())$gameVariables._data[305]=this.subject().param(4)
