...
Game_Action.prototype.isInsideAoeCircle = function(main, target) {
    ...
    if (mainX > targetX) {
      targetX = Math.min(mainX, target.aoeX() + target.aoeWidth() / 4);
    } else if (mainX < targetX) {
      targetX = Math.max(mainX, target.aoeX() - target.aoeWidth() / 4);
    }
    if (mainY > targetY) {
...
Game_Action.prototype.aoeRect = function(main) {
    ...
    if (skill.aoeRectAll) {
      ...
      rect.width = Graphics.boxWidth;
      rect.height = skill.aoeRectRow/5;
      rect.x = 0;
      rect.y = main.aoeY() - rect.height / 2 -50;
    }
    return rect;
};
