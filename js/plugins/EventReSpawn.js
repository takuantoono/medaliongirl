...
(function() {
    ...
    Game_Map.prototype.spawnEvent = function(originalEventId, x, y, isTemplate) {
        if ($gameMap.eventIdXy(x, y)>0&&y==4){
    ...
    Game_Map.prototype.spawnEventRandom = function(originalEventId, conditionMap, isTemplate) {
        ...
        if (position) {
        	...
        	var y = Math.randomInt(6)+4;
        	var did = $gameMap.eventIdXy(x, y);
        	if(did>0 && !$gameSelfVariables.value($gameMap._mapId, did, "coins"))$gameSelfVariables.setValue($gameMap._mapId, did, "coins", 1);
        	if(did>0 && originalEventId==$gameMap.event(did)._originalEventId && $gameSelfVariables.value($gameMap._mapId, did, "coins")<10){
        	$gameSelfVariables.setValue($gameMap._mapId, did, "coins", $gameSelfVariables.value($gameMap._mapId, did, "coins")+1);
        	}else{
        	if(!$gameSwitches._data[1])return false;
            if(originalEventId==1) $gameVariables._data[9]+=1;
            if(originalEventId==4) $gameVariables._data[10]+=1;
            if(originalEventId==23) $gameVariables._data[11]+=1;
...
if($gameSwitches._data[6])console.log(this)
