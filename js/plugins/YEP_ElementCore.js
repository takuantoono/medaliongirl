...
Game_Action.prototype.calcElementRate = function(target) {
  ...
  while (elements.length > 0) {
    ...
    //elementRateStateChanging
    if(elementId==6 && this.subject().isStateAffected(122))eleRate += 0.5;
    if(elementId==12 && this.subject().isStateAffected(122))eleRate += 0.5;
    if(elementId==4 && this.subject().isStateAffected(119))eleRate += 0.5;
    if(elementId==7 && this.subject().isStateAffected(119))eleRate += 0.5;
    if(elementId==11 && this.subject().isStateAffected(119))eleRate += 0.5;
    if(elementId==5 && this.subject().isStateAffected(120))eleRate += 0.5;
    if(elementId==9 && this.subject().isStateAffected(120))eleRate += 0.5;
    if(elementId==10 && this.subject().isStateAffected(120))eleRate += 0.5;
    if(elementId==16 && this.subject().isStateAffected(121))eleRate += 0.5;
    ...
    if((this.subject()._dualWieldSpriteIndex-1)>0){
        var wepid = this.subject()._dualWieldSpriteIndex - 1;
        }else{
        var wepid = 0;
        }
    if(this.subject()._equips[wepid]._itemId==0 && this._item._itemId==1){
      finalRate = finalRate || 1.00;
      finalRate -= 1.00;
      finalRate += target.elementRate(16);
     }
    }
    }
  ...
  if(target.isEnemy()&&$dataEnemies[target._enemyId].meta.ghost){
  elements = this.getItemElements()
    if(elements.indexOf(12) < 0 && elements.indexOf(13) < 0 && elements.indexOf(29) < 0){
