...
(function() {
    ...
    Game_Unit.prototype.getChainCount2 = function() {
        return this._chainCount2 || 0;
    };
    ...
    Game_Unit.prototype.getChainRate = function(addRate) {
    	var drate = param.damageRate;
    	if($gameVariables.value(255)==1)drate+=1
    	if(!$gameSwitches.value(1010))drate+=1
        return (100 + this.getChainCount() * drate * addRate).clamp(0, param.maxRate || Infinity) / 100;
    };
    ...
    Game_Action.prototype.makeDamageValue = function(target, critical) {
        ...
        if (this.isHpEffect()) {
        	var srate = 0;
        	if(this.subject().isStateAffected(317)&&this.friendsUnit().getChainRate(this.getAdditionalChainRate())>1){
        	srate += 0.2;
        	}
        	if(target.isStateAffected(316)&&this.friendsUnit().getChainRate(this.getAdditionalChainRate())>1){
            srate += 0.2;
            }
            damageValue *= this.friendsUnit().getChainRate(this.getAdditionalChainRate()+srate);
            damageValue+=(this.friendsUnit().getChainRate(this.getAdditionalChainRate())-1)*20
        }
    ...
    Game_Action.prototype.updateChain = function(target) {
        ...
        if (this._damageForChain) {
    		...
    		if(this._subjectEnemyIndex==BattleManager._enemyNum){
    		return;
    		}else{
    		BattleManager._enemyNum=this._subjectEnemyIndex;
    		BattleManager._actorNum=0;
    		};
    		}
    		if(this._subjectActorId>0){
    		if(this._subjectActorId==BattleManager._actorNum){
    		return;
    		}else{
    		BattleManager._actorNum=this._subjectActorId;
    		BattleManager._enemyNum=-1;
    		};
    		}
    		this._preUnit=this.subject();
            if(this.subject().isEnemy()&&$dataEnemies[this.subject()._enemyId].meta.mikata){
            }else{
        ...
        if (param.cancelOpposite) {
            if(!this.subject().isEnemy()&&$dataEnemies[this.subject()._enemyId].meta.mikata)this.opponentsUnit().resetChainCount();
        }
    };
