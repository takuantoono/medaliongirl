...
Game_Event.prototype.isCollidedWithEvents = function(x, y) {
  ...
  var events = $gameMap.eventsXyNt(x, y);
  return events.some(function(event) {
  	if($dataMap.events[event._eventId]){
  	if(!$dataMap.events[event._eventId].meta.rbox&&!$dataMap.events[event._eventId].meta.jade&&!$dataMap.events[event._eventId].meta.misril)return nya;
    };
    });
    }
  var chk2 = this.check_events_size(x,y);
