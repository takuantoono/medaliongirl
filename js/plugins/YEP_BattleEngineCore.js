...
Sprite_Damage.prototype.setup = function(target) {
    ...
    if (result.critical) {
      this.setupCriticalEffect(target);
    }
};
...
Window_BattleActor.prototype.autoSelect = function() {
    ...
    if (action.isForUser()||action.isForAllButUser()) {
...
var sp = false;
if(action){
if(action._item.isItem()){
if($dataItems[action._item._itemId].meta.deadalive)sp=true;
}
if(action._item.isSkill()){
if($dataSkills[action._item._itemId].meta.deadalive)sp=true;
}
}
    if (this._selectDead&&!sp) return this.actor().isDead();
