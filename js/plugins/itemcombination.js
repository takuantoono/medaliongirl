...
Game_Party.prototype.canCombine = function(recipeKeyName){
	...
	return (haveAllIngredients === $gameParty.ingredients(recipeKeyName).length)&&recipeDetails[1]<=$gameVariables.value(98);
};
...
craftingTitleWindow.prototype.drawCraftingTitle = function( x, y, width) {
    ...
    this.resetTextColor();
    this.drawText("Item Combination Menu  [AP " + $gameVariables.value(98) + " ]" , x, y, width - unitWidth - 6, 'left');
...
craftingDetailsWindow.prototype.drawComboChance = function(x, y, width) {
	...
	if(recipeDetails[1]>=$gameVariables.value(98)){
	this.changeTextColor(this.hpGaugeColor1())
	}
	this.drawText("AP" + chance, x, y, this.windowWidth() - 32 , 'center');
    this.changeTextColor(this.systemColor());
};
...
Scene_CraftingMenu.prototype.onDetailsOk = function() {
	...
	if($gameParty.canCombine(recipeKeyName)){
		...
		var aa =$gameVariables.value(98)-recipeDetails[1]
		$gameVariables.setValue(98,aa)
