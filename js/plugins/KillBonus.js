...
(function() {
    ...
    Game_Action.prototype.executeHpDamage = function(target, value) {
        ...
        var exp = $dataEnemies[target._enemyId].exp
        this.executeKillBonus(exp);
        }
    };

    Game_Action.prototype.executeKillBonus = function(exp) {
        ...
        subject.traitObjects().forEach(function(data) {
            ...
            this.executeKillBonusScript(data, subject, exp);
        }.bind(this));
    ...
    Game_Action.prototype.executeKillBonusScript = function(data, subject, exp) {
    	if (!subject) return;
        var value = getMetaValues(data, ['Script', 'スクリプト']);
        if (value) {
        $gameVariables.setValue(116, subject._name+'はEXP'+exp+'を獲得した！');
        var oldlevel = subject.level
        subject.changeExp(subject.currentExp() + exp, true);
        if(subject.level != oldlevel) $gameVariables.setValue(116, $gameVariables.value(116)+'　レベルアップ！');
            //try {
    ...
    if (target.result().used) {
       ...
       this.push('popBaseLine');
        ...
        this.push('addText', $gameVariables.value(116));
