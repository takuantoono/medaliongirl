...
BattleManager.processVictory = function() {
    ...
    $gameParty.resetChainCount();
        ...
        var group = $gameTroop.allMembers();
    var length = group.length;
    for (var i = 0; i < length; ++i) {
      var member = group[i];
      if ($dataEnemies[member._enemyId].meta.mikata) {
      if ($dataEnemies[member._enemyId].meta.otomo)$gameVariables._data[911]=member._hp
      member._hidden=true;
      }
    }
...
Scene_Battle.prototype.updateVictoryDrops = function() {
    if (!this._victoryDropWindow) {
      ...
      AudioManager.playSe({"name":"one22","volume":90,"pitch":100,"pan":0})
