...
Yanfly.Param.slotName1 = String(Yanfly.Parameters['Slot Name1']);
Yanfly.Param.slotName2 = String(Yanfly.Parameters['Slot Name2']);
...
ItemManager.processAugmentEffect = function(line, mainItem, effectItem, slot) {
  ...
  if(DataManager.isWeapon(mainItem)){
  // ADD ATTACK P STATE: x
  if (line.match(/ADD ATTACK P STATE:[ ](.*)/i)) {
    var text = String(RegExp.$1).toUpperCase().trim();
    return this.applyAugmentAttackState(mainItem, text, true);
  } else if (line.match(/REMOVE ATTACK P STATE:[ ](.*)/i)) {
    var text = String(RegExp.$1).toUpperCase().trim();
    return this.applyAugmentAttackState(mainItem, text, false);
  }
  }
  ...
  // ADD ELEMENT G RATE: x
  if (line.match(/ADD ELEMENT G RATE:[ ](.*)/i)) {
    var text = String(RegExp.$1).toUpperCase().trim();
    return this.applyAugmentElement(mainItem, text, true);
  } else if (line.match(/REMOVE ELEMENT G RATE:[ ](.*)/i)) {
    var text = String(RegExp.$1).toUpperCase().trim();
    return this.applyAugmentElement(mainItem, text, false);
  }
  }
  if(DataManager.isWeapon(mainItem)){
  // ADD ELEMENT P RATE: x
  if (line.match(/ADD ELEMENT P RATE:[ ](.*)/i)) {
    var text = String(RegExp.$1).toUpperCase().trim();
    return this.applyAugmentElementP(mainItem, text, true);
  } else if (line.match(/REMOVE ELEMENT P RATE:[ ](.*)/i)) {
    var text = String(RegExp.$1).toUpperCase().trim();
    return this.applyAugmentElementP(mainItem, text, false);
  }
  }
  // ADD PASSIVE STATE: x
  ...
  if(DataManager.isArmor(mainItem)){
  // ADD STATE G RATE: x
  if (line.match(/ADD STATE G RATE:[ ](.*)/i)) {
    var text = String(RegExp.$1).toUpperCase().trim();
    return this.applyAugmentStateRate(mainItem, text, true);
  } else if (line.match(/REMOVE STATE G RATE:[ ](.*)/i)) {
    var text = String(RegExp.$1).toUpperCase().trim();
    return this.applyAugmentStateRate(mainItem, text, false);
  }
  }
...
ItemManager.applyAugmentElementP = function(mainItem, text, add) {
    if (text.match(/(\d+),[ ](\d+)([%％])/i)) {
      var id = parseInt(RegExp.$1);
      var rate = parseFloat(RegExp.$2) * 0.01;
    } else if (text.match(/(.*),[ ](\d+)([%％])/i)) {
...
Window_ItemActionCommand.prototype.addAugmentSlots = function() {
    ...
    for (var i = 0; i < length; ++i) {
      ...
      var str = new String(slot);
      var slot1 = new String(Yanfly.Param.slotName1);
      var slot2 = new String(Yanfly.Param.slotName2);
      console.log(Yanfly.Param.slotName1)
      if(str[0]==slot1[0]){
      slot= "God Vein";
      }
      if(str[0]==slot2[0])slot= "Gear";
      }
...
Window_AugmentItemList.prototype.containsType = function(item) {
    ...
    var length = this._item.augmentSlotItems.length;
    for (var i = 0; i < length; ++i) {
      var text = this._item.augmentSlotItems[i];
      if (text.match(/ITEM[ ](\d+)/i)) {
        var id = parseInt(RegExp.$1);
        var augment = $dataItems[id];
        if(item.meta.shinmyaku == 1 && item.id != augment.id)return false
      }
    }
    if(!item.meta.shinmyaku && DataManager.isWeapon(this._item) && item.meta.aarmor)return false
    if(!item.meta.shinmyaku && DataManager.isArmor(this._item) && !item.meta.aarmor)return false
    if(!item.meta.shinmyaku && this._item.atypeId != 5 && this._item.atypeId != 6 && item.meta.ashield)return false
...
Scene_Item.prototype.createAugmentListWindow = function() {
    ...
    this._augmentListWindow.setHandler('description', this.descriptionOpen.bind(this));
