...
Window_QuickNotification.prototype.setDestinationY = function(y)
{
    ...
    this._destinationY = y-50;
    if(y>400)this._destinationY -= 50;
    if(y>300)this._destinationY -= 50;
...
Window_QuickNotification.prototype.update = function()
{
    ...
    if($gameSwitches.value(294)){
    if(this.isFadingIn())this._duration--;
    }else{
    this._duration--;
    }
    if($gameSwitches.value(295))this._duration=0;
    this.updateOpacity();
    this.updatePosition();
...
Scene_Base.prototype.createQuickNotificationWindow = function()
{
    ...
    var xx = 50;
    var yy = 120;
    var x = (Graphics.width  - width)  / 2 +50;
    var y = (Graphics.height - height) / 2 +120;
...
Scene_Base.prototype.displayQuickNotification = function()
{
    ...
    if($gameSwitches.value(296)){this._quickNotificationWindowLayer.move(420, 0, 800, 600);}
    // 通知音を再生
