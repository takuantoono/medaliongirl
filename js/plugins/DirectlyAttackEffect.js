...
(function() {
    ...
    var SFrate                = getParamNumber(['rate', 'レート']);
...
//sf追加
    ...
    return target.isDyingSF() && !this._action.isCertainHit();
    };

    Game_BattlerBase.prototype.isDyingSF = function() {
    var val1 = SFrate / 100;
    return this.isAlive() && this._hp < this.mhp * val1;
    };
    ...
    if($gameSystem.isSideView() && target.isActor()){
      if(BattleManager._action.isForAll()) {
        this.requestMotion('skill');    
      }else{
        SubsSubs = this;
        Substarget = target;
        //console.log(SubsSubs)
        //console.log(Substarget)
      }
    }
    };

    var _Sprite_Battler_updateDirectly = Sprite_Battler.prototype.updateDirectly;
    Sprite_Battler.prototype.updateDirectly = function() {
    _Sprite_Battler_updateDirectly.call(this);
    this.stepSF();
    };

    Sprite_Battler.prototype.stepSF = function() {
    if(SubsSubs){
    var spriteSet = this.parent.parent.parent;
    var target = Substarget;
    var a = spriteSet.SubstargetSprite(target);
    var target = SubsSubs;
    var b = spriteSet.SubstargetSprite(target);
    var xx = a._homeX - b._homeX - 80; //128 == 64 * 1.25
    var yy = a._homeY - b._homeY + 24; //32 <= 64 * 0.38
    //console.log(xx)
    b.startMove(xx, yy, 15);
    SubsSubs = null;
    }
    };

    Spriteset_Battle.prototype.SubstargetSprite = function(target) {
    var val1 = $gameParty.battleMembers().indexOf(target);
    //console.log(this._actorSprites[val1])
     return this._actorSprites[val1];
    };
