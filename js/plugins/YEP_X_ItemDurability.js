...
Game_Actor.prototype.damageRandomDurability = function(value, group) {
    ...
    if(item.atypeId==5||item.atypeId==6){
      if(Math.randomInt(3)==1 && this.isStateAffected(103))return;
    }
...
Game_Actor.prototype.damageWeaponDurability = function(item, num) {
    ...
    if (!item) return;
    item.durability -= 1;
    if(!this.isStateAffected(102))item.durability -= Math.randomInt(2);
...
Window_ItemInfo.prototype.drawItemDurability = function(dy) {
    ...
    if(!$gameSwitches.value(376))text = "Durability";
...
Window_ItemInfo.prototype.drawItemBuseki = function(dy) {
    ...
    if(!$gameSwitches.value(376))text = "Object's Soul";
    var dx = this.textPadding();
    var dw = this.contents.width - this.textPadding() * 2;
    this.drawText(text, dx, dy, dw);
    var fmt = Yanfly.Param.IDurFmt;
...
Window_ItemActionCommand.prototype.addRepairCommand = function() {
    ...
    if(!$gameSwitches.value(376))fmt = "Repair";
